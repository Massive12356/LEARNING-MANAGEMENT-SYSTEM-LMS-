...
import { coursePerformanceAnalytics, teacherAnalyticsStats } from '../../types';
...
interface TeacherReport {
  ...
  overallStats: teacherAnalyticsStats;
...
export function TeacherReports() {
  ...
  const [courseData, setCourseData] = useState<teacherAnalyticsStats | null>(null);
  const [recentData, setRecentData] = useState([]);
  const [coursePerformance, setCoursePerformance] = useState<coursePerformanceAnalytics[]>([]);
  ...
  const loadCourseStats = async () => {
    ...
    try {
      const response: teacherAnalyticsStats = await courseService.loadTeacherAnalytics();
  ...
  const loadActivities = async () => {
    ...
    try {
      const response = await courseService.loadRecentActivities();
  ...
  const loadPerformance = async () => {
    try {
      const response = await courseService.loadCoursePerformance();
      setCoursePerformance(response);
      return response;
    } catch (error: any) {
    ...
    if (format === 'csv') {
      ...
      const csvData = [
        ...
        { Metric: 'Total Students', Value: reportData.overallStats.totalStudents },
        {
          ...
          Value: `${reportData.overallStats.averageCompletionRate.toFixed(1)}%`,
        },
        {
          Metric: 'Total Certificates Issued',
          Value: reportData.overallStats.totalCertificatesIssued,
        },
      ];
...
<body>
    ...
    <div class="section">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">${reportData.overallStats.averageCompletionRate.toFixed(
                  1
                )}%</div>
          ...
          <CardContent className="flex items-center p-6">
            ...
            <div className="ml-4">
              {loading ? (
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">
                  {courseData?.averageCompletionRate?.toFixed(1) ?? 0.0}%
                </p>
              )}
          ...
          <CardContent className="flex items-center p-6">
            ...
            <div className="ml-4">
              {loading ? (
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">
                  {courseData?.totalCertificatesIssued ?? 0}
                </p>
              )}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                {/* let backend return id in the payload */}
                      ...
                      <div className="flex items-center">
                        <div>
                          ...
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            Status: {course?.courseStatus ?? 'N/A'}
                          </div>
                        </div>
                      </div>
                    </td>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <EmptyState
                ...
                description="Student activities will appear here once learners start interacting with your courses."
              />
            ) : (
              // any type should be removed when you get the coreect values from the backend
              recentData?.map((activity: any, index) => (
                <div key={index} className="flex items-center space-x-4">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      activity.action === 'completed'
                        ? 'bg-green-100 dark:bg-green-900'
                        : activity.action === 'certificate_earned'
