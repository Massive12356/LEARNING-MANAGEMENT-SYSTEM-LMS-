...
export const VerificationCodePage: React.FC<VerificationCodePageProps> = ({ email, onCodeVerified }) => {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (/^\d{1,4}$/.test(pasteData)) {
      const newCode = pasteData.split('').concat(Array(4 - pasteData.length).fill('')).slice(0, 4);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(`[UI] Attempting to verify code: ${codeString} for email: ${email}`);
